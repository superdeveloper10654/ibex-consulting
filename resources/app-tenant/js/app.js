...
(function ($) {
    ...
    function notificationsFunctional() {
        $(document).on('click', '[data-action="dismiss-notification"]', function () {
            let notif_id = $(this).data('notification-id');
            let notif_el = $(this).closest('.list-group-item');

            notif_el.css('transition', '.35s');
            notif_el.addClass('remove-step1');

            setTimeout(() => {
                notif_el.addClass('remove-step2');
            }, 200);

            $.post(conf.routes['notifications.dismiss'], { id: notif_id }, function (res) {
                if (!res.success) {
                    errorMsg(res.message);
                    notif_el.removeClass('remove-step1 remove-step2');
                } else {
                    let counter_value = Number($('#topbar-notification-counter').text());
                    if (counter_value > 0) {
                        $('#topbar-notification-counter').text(counter_value - 1);
                    }
                    notif_el.remove();
                }
            });
        ...
        $(document).on('click', '[data-action=dismiss-all-notifications]', function () {
            $('#topbar-notification-counter').text(0);
            let timeout_step = 20;
            let timeout = 0;

            $.each($(document).find('[data-action="dismiss-notification"]'), (i, el) => {
 ...
 * @return {Promise}
 */
function myAjax(params_or_url, maybe_data = [])
{
    return new Promise((resolve, reject) => {
...
/**
 * Set value = 1 for is_draft element and submit the form
 *
 * @param {jQuery} el
 */
function saveAsDraft(el)
{
    $(el).closest('form').find('[name=is_draft]').val(1);
...
/**
 * Change elements array name indexes
 *
 * @param {object} elems
 * @param {integer} i
 * @param {string} attr name to replace
 */
function changeElemsIndexes(elems, i, attr = 'name') {
    $(elems).each(function () {
...
$(document).ready(function () {
    let $expand_toggler = $('.header-wrapper .btn.toggle-page-view');

    $expand_toggler.on('click', function (e) {
        toggleResourcePageView(this);
    });

    if (localStorage.getItem(storage_const.RESOURCE_PAGE_VIEW) == 'expanded') {
        if ($expand_toggler.length) {
            toggleResourcePageView($expand_toggler[0]);
        }
    }
});

function toggleResourcePageView(el) {
    let $parent = $(el).closest('.row');
    let $left_col = $parent.find('> .left-col');
    let $right_col = $parent.find('> .right-col');

    $left_col.toggleClass('w-100');
    $right_col.toggleClass('w-100');

    if ($left_col.hasClass('w-100')) {
        localStorage.setItem(storage_const.RESOURCE_PAGE_VIEW, 'expanded');
    } else {
        localStorage.setItem(storage_const.RESOURCE_PAGE_VIEW, 'collapsed');
    }
}

function isRgb(maybe_rgb) {
    return maybe_rgb.startsWith('rgb(') && maybe_rgb.endsWith(')');
}

function isHex(maybe_hex) {
    return maybe_hex.startsWith('#') && maybe_hex.length == 7;
}

function rgbToHex(rgb) {
    let components = rgb.replace('rgb(', '')
        .replace(')', '')
        .split(',');
...
/**
 * Remove item from database and from table
 *
 * @param {string} url
 * @param {jQuery} clicked_el
 * @param {boolean|function} callback
 */
function removeItem(url, clicked_el, callback = false) {
    $(clicked_el).closest('tr').fadeOut(250);
    ...
    $.ajax({
        ...
        complete: (res) => {
            ...
            if (!res.success) {
                errorMsg(res.message);
                $(clicked_el).closest('tr').fadeIn(250);
                return;
            }
