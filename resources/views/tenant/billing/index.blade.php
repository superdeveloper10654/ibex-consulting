...
<input type="hidden" name="billableId" value="{{ t_profile()->id }}" />
<input type="hidden" name="billableType" value="user" />
<input type="hidden" name="country" value="GB" />
<input type="hidden" name="postal_code" value="{{ t_profile()->billing_postal_code }}" />
<input type="hidden" name="vat_number" value="{{ t_profile()->vat_id }}" />
                ...
                <div class="card">
                    <div class="card-body p-4">
                        ...
                        @if (!empty($plan->features)) 
                            <div class="plan-features mb-4 text-start">
            ...
            @if ($selected)
                <div class="next-payment-date-wrapper row justify-content-center">
                    <div class="col-md-6 card p-5">
                        ...
                        <p>This is for your {{ t_profile()->team_users_count }} users.</p>

                        @if ($selected_plan && !empty(t_profile()->card_last_four) && !empty(t_profile()->card_expiration))
                            <p class="text">Your current payment method is a <span style="text-transform: capitalize;">{{ ( t_profile()->card_brand ) }} </span> card ending in {{ t_profile()->card_last_four . ' ' . __('which expires on') . ' ' . t_profile()->card_expiration }}.</p>
                        @endif
                    </div>
                </div>
            @endif
          ...
          <p class="text-muted text-center">Please enter your billing details below </p>
            ...
            <div class="mb-3">
                    ...
                    <input id="users_count" name="users_count" type="number" 
                        style="width: 100px;"
                        class="form-control" 
                        value="{{ t_profile()->team_users_count }}" 
...
@section('script')
    <script>
        let payment_method_added = Boolean({{ $selected_plan && !empty(t_profile()->card_last_four) && !empty(t_profile()->card_expiration) }});
        ...
        window.onload = () => {
            @if ($state != 'onGracePeriod')
                ...
                $('.plan-box').each((i, el) => {
                    ...
                    // subscribe on plan
                    <?php /* plan_el.find('.subscribe').on('click', function(e) {
                        if ($(this).hasClass('demo')) {
                            showLoader();
                            infoMsg('Configuring your instance ... It can take a while');
                            request('POST', '/billing/subscribe-demo').then(response => {
